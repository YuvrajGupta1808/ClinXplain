...
def _(mo):
    ...
    - **Embeddings**: Always use OpenAI (`OPENAI_API_KEY`) for RAG and ShortTermMemory — W&B Inference has no /embeddings endpoint.
    - **LLM/Chat**: When `WANDB_API_KEY` is set, LLM uses [W&B Inference](https://api.inference.wandb.ai/v1) (models: `openai/gpt-oss-20b`, `openai/gpt-oss-120b`). Otherwise uses OpenAI (`gpt-4o-mini`).
    - Set `WANDB_LLM_MODEL=openai/gpt-oss-20b` (or `openai/gpt-oss-120b`) to override the W&B model.
    """)
    return
...
def _():
    ...
    # Embeddings: ALWAYS use OpenAI (RAG + ShortTermMemory need embeddings; W&B has no /embeddings)
    os.environ["OPENAI_API_KEY"] = os.getenv("OPENAI_API_KEY", "")

    # LLM: use W&B Inference when WANDB_API_KEY is set; else OpenAI
    # Do NOT set OPENAI_API_BASE globally — that breaks embeddings (404 on W&B /embeddings)
    wandb_api_key = os.getenv("WANDB_API_KEY")
    if wandb_api_key:
        _llm_backend = "W&B Inference"
        _llm_base_url = "https://api.inference.wandb.ai/v1"
        _llm_api_key = wandb_api_key
        _llm_model = os.getenv("WANDB_LLM_MODEL", "openai/gpt-oss-20b")
    else:
        _llm_backend = "OpenAI"
        _llm_base_url = None
