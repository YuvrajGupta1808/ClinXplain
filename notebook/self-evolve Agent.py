...
def _():
    ...
    # Import API keys from .env (OpenAI and WandB)
    ...
    if not WANDB_API_KEY:
        print("WARNING: WANDB_API_KEY not found. Set it in .env for Weave/WandB.")
    else:
        print("SUCCESS: WANDB_API_KEY loaded from .env")

    # Use WandB Inference for LLM calls (base URL + API key)
    os.environ["OPENAI_API_BASE"] = "https://api.inference.wandb.ai/v1"
    if WANDB_API_KEY:
        os.environ["OPENAI_API_KEY"] = WANDB_API_KEY
    return (
        Any,
        BaseModel,
        ChatOpenAI,
...
async def _(
    ...
    print("=" * 60)
    print(f"View detailed results in WandB:")
    print(f"🔗 https://wandb.ai/pthebe-san-francisco-state-university/differential-diagnosis")
    print("\nCharts will show:")
