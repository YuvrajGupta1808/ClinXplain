"""
RAG evaluation and tracing with W&B Weave.

Follows the WandB Weave RAG tutorial:
...
def _get_retrieved_context(
    ...
    docs = retrieve(question, config=cfg, top_k=k)
...
if _WEAVE_AVAILABLE:
    ...
    def get_retrieved_context(question: str) -> str:
        ...
        Returns concatenated context from vector search (RedisVL).
        """
...
verdict: """
    ...
    # See https://docs.ragas.io/ and https://docs.wandb.ai/weave/tutorial-rag
    try:
    ...
    def _default_weave_project() -> str:
        """Use WEAVE_PROJECT env, or infer entity from logged-in W&B user (wandb.api.viewer())."""
        env = os.getenv("WEAVE_PROJECT", "").strip()
        if env:
            return env
        try:
            import wandb
    ...
    def init_weave(project: str | None = None) -> None:
        ...
        raw = (project or os.getenv("WEAVE_PROJECT") or "").strip()
        # Never use placeholder "clinxplain" — use W&B viewer entity if that was set
        if raw.startswith("clinxplain/"):
            raw = ""
        entity_project = raw or _default_weave_project()
        if not entity_project:
            raise ValueError(
                "Weave project (entity/project) is required. Set WEAVE_PROJECT in .env to your W&B entity and project, e.g.:\n"
                "  WEAVE_PROJECT=your-username/rag-eval\n"
                "Replace 'your-username' with your W&B username (see https://wandb.ai/settings)."
            )
    ...
    def run_evaluation(
        ...
        if not _WEAVE_AVAILABLE:
            raise ImportError("weave is required for evaluation. Install with: pip install weave wandb")
