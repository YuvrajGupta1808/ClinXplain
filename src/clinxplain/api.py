...
class IngestResponse(BaseModel):
    ...
    keys_count: int = Field(..., description="Number of Redis keys written")
...
async def websocket_chat(websocket: WebSocket) -> None:
    ...
    try:
        ...
        docs = pipeline.retrieve(effective_query, top_k=config.top_k)
        context_str = "\n\n---\n\n".join(
            (d.page_content or "").strip() for d in docs
        ) if docs else "No relevant documents found."
        system = (
            "Answer based on the following context. If the context does not contain enough information, say so. Do not make up information."
        )
...
async def ingest_documents(
    ...
    """
    Ingest uploaded documents into the RAG index (RedisVL).
